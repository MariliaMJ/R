...
library(lubridate)
#load data
source_file <- "https://s3.amazonaws.com/udacity-hosted-downloads/ud651/prosperLoanData.csv"
loans <- read.csv(source_file)
```
...
loans$LoanOriginationDate.month <- loans$LoanOriginationDate %>%
  format('%b')
loans$LoanOriginationDate.month <- factor(loans$LoanOriginationDate.month,
                                            months)
ggplot(loans, aes(LoanOriginationDate.month)) +
...
```{r, fig.width = 10}
qplot(x = IncomeRange, data = loans, fill = I("#0077C0")) + ggtitle("Loans per income range")
```
...
ggplot(data = loans, aes(ListingCategory..numeric.)) + 
  geom_bar(fill = I('#0077C0')) +
...
```
The distribution is positevely skewed. New York and California are usually maong the top 10 most expensives states to live in the USA, it matches in the expectation that some states taht require a higher economic level would have more loans.
...
```{r, fig.width = 10}
CreditScoreCount <- qplot(x = CreditScoreRangeLower, data = loans, binwidth = 20,
      color = I("#424242"), fill = I("#0077C0")) +
  scale_x_continuous(limits = c(400, 900), breaks = seq(400, 900, 50))
CreditScoreDensity <- ggplot(aes(x = CreditScoreRangeLower), data = d) +
  geom_histogram(aes(y = ..density..),
                 binwidth = 20,
                 color = I("#424242"), fill = I("#0077C0")) +
  geom_density(adjust = 4, alpha = 0.3, fill = I("#DE703B")) +
  geom_vline(aes(xintercept = mean(CreditScoreRangeLower, na.rm = T)),
...
with(loans, cor.test(LoanOriginalAmount, CreditScoreRangeLower, 
                        method = "pearson"))
```
